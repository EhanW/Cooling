...
def get_args():
    ...
    parser = argparse.ArgumentParser('CIFAR10-PGD-AT-DEL')
    ...
    for epoch in range(args.epochs):
        ...
        for id, (data, target, index) in enumerate(train_loader):
            ...
            if epoch + 1 >= args.del_start_epoch:
                indices = loss > args.del_threshold
                print(
                    epoch, id, indices.sum().item(), len(data)
                )
                loss *= indices
            loss = loss.mean()
            optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    path = os.path.join('logs/del/'+args.model_name, f'{args.del_threshold}_{args.del_start_epoch}')
